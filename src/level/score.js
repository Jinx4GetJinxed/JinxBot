...
async function level_up_message(client, message, level) {
  ...
  message.reply({
    embeds: [
      {
        color: randomColor(),
        description: `\`${message.guild.tagname}\`, tu as atteint le niveau \`${score.level}\` et tu viens de passer au rang ${level} !!!`,
      },
    ],
  });
}
...
function score_give(message, client, args) {
  ...
  return message.channel.send(
    `${user.tag} a gagné ${pointsToAdd} points d'exp et mtn il a ${userScore.points} points.`
  );
}
...
module.exports = { score_add, score_give, show_level, top_rank, remove_points };
